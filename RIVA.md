...
I built RIVA because SBOMs and build signatures don't answer "what's actually running right now?" It's a Windows agent plus a Python verifier that produces signed runtime evidence and compares it against a known baseline. In a lab run on 25–26 September 2025, it caught four unauthorised npm packages and a scripted exfiltration while the service's health checks stayed green.
...
- *Verifier Service*: Python service (FastAPI), REST API, Redis for ML cache, SQLite for metrics, a rule engine that scores SARs against SBOM and baseline policy. Findings are forwarded to an external persistence layer.
- *Baseline Policy Store*: signed manifests and imported SBOMs, managed through the CLI tools.
- *Evidence & Alerts*: findings store with JSON exporters and Prometheus metrics.
- *Operator Consoles*: CLI tools (`rim-monitor`, `rim-api`) plus a React web UI for baseline import and findings review.
...
*Command:* `rimctl list-findings --since 1h` *(CLI tool name at time of demo)*
...
- Evidence bundle export: package findings, transcripts, and screenshots into a single artefact via CLI.
